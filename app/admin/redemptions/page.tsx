...
export default function RedemptionsPage() {
    ...
    const load = async (status: Tab) => {
        ...
        const { data } = await supabase
            .from('redemptions')
            .select('*, profiles(name, email, total_points), rewards(title, points_required)')
            .eq('status', status)
    ...
    return (
        <div className="space-y-6">
          ...
          ${toast.type === 'success' ? 'bg-emerald-950 border-emerald-500/40 text-emerald-300' : 'bg-red-950 border-red-500/40 text-red-300'}`}>
            ...
            <div>
                <h1 className="text-2xl font-bold text-zinc-100">Redemptions</h1>
                <p className="text-zinc-400 text-sm mt-1">Approve reward redemption requests</p>
            </div>
                        ...
                        <tbody className="divide-y divide-zinc-800/50">
                            ...
                            {(redemptions as any[]).map(r => (
                                <tr key={r.id} className="hover:bg-zinc-900/40 transition-colors">
                                    ...
                                    <td className="px-4 py-4 text-zinc-300">{r.rewards?.title}</td>
                                    <td className="px-4 py-4">
                                        <span className="font-semibold text-amber-400">{r.rewards?.points_required}</span>
                                    ...
                                    {tab === 'pending' && (
                                        <td className="px-4 py-4">
                                            <button
                                                ...
                                                onClick={() => approve(r)}
                                                className="flex items-center gap-1.5 text-xs font-medium text-emerald-400 hover:bg-emerald-500/10 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                                            >
                                    ...
                                    {tab === 'approved' && (
                                        <td className="px-4 py-4">
                                            <button
                                                ...
                                                className="flex items-center gap-1.5 text-xs font-medium text-violet-400 hover:bg-violet-500/10 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                                            >
