    ...
    return (
        <div className="space-y-6">
          ...
          ${toast.type === 'success' ? 'bg-emerald-950 border-emerald-500/40 text-emerald-300' : 'bg-red-950 border-red-500/40 text-red-300'}`}>
            ...
            <div>
                <h1 className="text-2xl font-bold text-zinc-100">Review Approvals</h1>
                <p className="text-zinc-400 text-sm mt-1">Approve or reject submitted Google reviews</p>
            </div>
                            ...
                            {reviews.map((r: any) => (
                                <tr key={r.id} className="hover:bg-zinc-900/40 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="font-medium text-zinc-200">{r.profiles?.name}</div>
                                        <div className="text-xs text-zinc-500">{r.profiles?.email}</div>
                                    </td>
                                    <td className="px-4 py-4">
                                        <a href={r.review_link} target="_blank" rel="noopener noreferrer"
                                            className="flex items-center gap-1.5 text-indigo-400 hover:text-indigo-300 transition-colors truncate max-w-[180px]">
                                    ...
                                    <td className="px-4 py-4">
                                        <button
                                            ...
                                            className="flex items-center gap-1.5 text-violet-400 hover:text-violet-300 transition-colors text-xs"
                                        >
                                            <Eye className="w-3.5 h-3.5" /> Preview
                                        </button>
                                    </td>
                                    <td className="px-4 py-4 text-zinc-400">{new Date(r.created_at).toLocaleDateString()}</td>
                                    <td className="px-4 py-4">
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex items-center gap-2">
                                                <button
                                                    ...
                                                    onClick={() => approve(r)}
                                                    className="flex items-center gap-1 text-xs font-medium text-emerald-400 hover:bg-emerald-500/10 px-2.5 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                                                >
                                                    {actionLoading === r.id
                                                ...
                                                <button
                                                    ...
                                                    className="flex items-center gap-1 text-xs font-medium text-red-400 hover:bg-red-500/10 px-2.5 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                                                >
            ...
            {previewUrl && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/90 backdrop-blur-sm" onClick={() => setPreviewUrl(null)}>
                    <div className="relative max-w-2xl w-full" onClick={e => e.stopPropagation()}>
                        ...
                        <img src={previewUrl} alt="Review screenshot" className="w-full rounded-2xl shadow-2xl border border-zinc-700" />
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/75 backdrop-blur-sm">
                    <div className="glass-card rounded-2xl p-6 w-full max-w-md shadow-2xl">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-base font-bold text-zinc-100">Reject Review</h2>
                            <button onClick={() => setRejectModal(null)} className="text-zinc-500 hover:text-zinc-300"><X className="w-4 h-4" /></button>
                        </div>
                        <p className="text-zinc-400 text-sm mb-4">Rejecting submission from <strong className="text-zinc-200">{rejectModal.name}</strong>. Optionally add a reason:</p>
                        <textarea
                            value={rejectReason}
                            onChange={e => setRejectReason(e.target.value)}
                            placeholder="Reason for rejection (optional)"
                            rows={3}
                            className="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 text-sm resize-none"
                        />
                        <div className="flex gap-3 mt-4">
                            <button onClick={() => setRejectModal(null)} className="flex-1 px-4 py-2.5 rounded-xl border border-zinc-700 text-zinc-300 hover:bg-zinc-800 text-sm font-medium">Cancel</button>
