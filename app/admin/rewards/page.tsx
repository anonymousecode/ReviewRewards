...
export default function RewardsPage() {
    ...
    const filteredRewards = rewards.filter(r => tab === 'enabled' ? r.is_active : !r.is_active)
        ...
        <div className="space-y-6">
          ...
          ${toast.type === 'success' ? 'bg-emerald-950 border-emerald-500/40 text-emerald-300' : 'bg-red-950 border-red-500/40 text-red-300'}`}>
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Manage Rewards</h1>
                    <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-1">Create and configure reward catalog</p>
                </div>
                <button
            ...
            {/* Tabs */}
            <div className="flex gap-1 bg-zinc-200/50 dark:bg-zinc-900/80 p-1 rounded-xl w-fit border border-zinc-300/50 dark:border-zinc-800/50 backdrop-blur-sm">
                {(['enabled', 'disabled'] as const).map(t => (
                    <button
                        key={t}
                        onClick={() => setTab(t)}
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {loading && [...Array(3)].map((_, i) => (
                    <div key={i} className="h-40 bg-zinc-200 dark:bg-zinc-800/60 rounded-2xl animate-pulse border border-zinc-300/50 dark:border-zinc-700" />
                ))}
                {!loading && filteredRewards.length === 0 && (
                    <div className="col-span-full glass-card rounded-2xl py-16 text-center text-zinc-500">
                        <Gift className="w-10 h-10 mx-auto mb-3 opacity-40" />
                        <p>No {tab} rewards found.</p>
                    </div>
                )}
                {filteredRewards.map(r => (
                    <div key={r.id} className="glass-card rounded-2xl p-5 flex flex-col gap-4 transition-all">
                  ...
                  ${r.is_active ? 'text-red-500 dark:text-red-400 hover:bg-red-500/10' : 'text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10'}`}
                            >
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-zinc-200 dark:border-zinc-800">
                            ...
                            <span className="text-lg font-bold text-indigo-500 dark:text-indigo-400">{r.points_required}</span>
                        </div>
                    </div>
                ))}
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/75 backdrop-blur-sm">
                    <div className="glass-card rounded-2xl p-6 w-full max-w-md shadow-2xl">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">Create Reward</h2>
                        ...
                        <form onSubmit={create} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">Reward Title</label>
                                <input
                                    type="text" required
                                    value={form.title}
                                    onChange={e => setForm(p => ({ ...p, title: e.target.value }))}
                                    placeholder="e.g. ₹100 Gift Voucher"
                                    className="w-full bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-zinc-900 dark:text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm"
                                />
                            </div>
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-zinc-900 dark:text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm resize-none"
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">Points Required</label>
                                <input
                            ...
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => setModalOpen(false)}
                                    className="flex-1 px-4 py-2.5 rounded-xl border border-zinc-300 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-sm font-medium">
                                    Cancel
                                </button>
