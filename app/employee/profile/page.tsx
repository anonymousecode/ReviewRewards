        ...
        return (
            <div className="space-y-6 animate-pulse">
                ...
                <div className="grid grid-cols-2 gap-4">
                    {[...Array(4)].map((_, i) => <div key={i} className="h-28 bg-zinc-800 rounded-2xl" />)}
                </div>
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="glass-card rounded-2xl p-6 flex flex-col sm:flex-row items-center sm:items-start gap-6">
                ...
                <div className="text-center sm:text-left flex-1">
                    ...
                    <div className="flex flex-wrap gap-3 mt-3 justify-center sm:justify-start">
                        ...
                        <div className="bg-indigo-500/15 border border-indigo-500/30 rounded-xl px-4 py-1.5">
                            <span className="text-indigo-400 font-bold text-lg">{profile?.total_points}</span>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatsCard title="Total Points" value={profile?.total_points ?? 0} icon={Award} iconColor="text-indigo-400" />
                <StatsCard title="Your Rank" value={rank > 0 ? `#${rank}` : '—'} icon={TrendingUp} iconColor="text-amber-400" />
                <StatsCard title="Approved Reviews" value={totalApproved} icon={CheckCircle} iconColor="text-emerald-400" />
                <StatsCard title="This Month" value={monthlyApproved} icon={Star} iconColor="text-violet-400" subtitle="approved reviews" />
            </div>
                        ...
                        <tbody className="divide-y divide-zinc-800/50">
                            ...
                            {reviews.slice(0, 10).map(r => (
                                <tr key={r.id} className="hover:bg-zinc-900/40 transition-colors">
                                    ...
                                    <td className="py-3 text-zinc-500 text-xs">{r.rejection_reason ?? ''}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
